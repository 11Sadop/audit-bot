...
def pledge_custom(call):
    ...
    a = database.get_auction(aid)
    c = cur(a['currency'])
    bot.edit_message_text(f"Pledged! Enter amount (min: {a['current_price']+a['min_increment']:,} {c}):", call.message.chat.id, call.message.message_id, reply_markup=mk)
